...
export default class Solver11 extends BaseSolver<number> {
  ...
  protected solvePart1(input: number): string {
    ...
    const SQUARE_SIZE = 3;
    for (let y = 1; y <= this.SIZE - SQUARE_SIZE + 1; y++) {
      for (let x = 1; x < this.SIZE - SQUARE_SIZE + 1; x++) {
  ...
  private createGrid(serialNumber: number): number[][] {
    const grid: number[][] = new Array<number[]>(this.SIZE + 1);
    for (let i = 0; i < grid.length; i++) {
      grid[i] = new Array<number>(this.SIZE + 1);
    }
